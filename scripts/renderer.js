...
hexo.extend.generator.register("lunr", function(locals) {
  ...
  items.forEach(function(post) {
    if (post.date._isAMomentObject) {
      year1 =
        language == "en" ? post.date.format("YYY") : post.date.format("YYYY");
    } else {
      year1 =
        language == "en"
          ? post.date.format("YYY")
    ...
    res[yearKey].forEach(function(post) {
      ...
      store["/" + post.path] = {
        ...
        authorLink:
          ...
          (hexo.config.author && hexo.config.author.link) ||
          (hexo.theme.author && hexo.theme.author.link) ||
          "/",
        authorNick:
          (post.author && post.author.nick) ||
          (hexo.config.author && hexo.config.author.name) ||
          (hexo.theme.author && hexo.theme.author.name) ||
          hexo.config.default_author_name ||
          "unknow"
      };
    });
    finalData.push({
