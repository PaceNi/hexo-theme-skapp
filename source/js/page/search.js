(function() {
  ...
  SearchPage.prototype = {
    init: function() {
      ...
      this.tpl = [
        ...
        this.config.language == "en"
          ? "Find <em>{{ num }}</em> {{ enDescription }} <em>{{ query }}</em>"
      ...
      this.tagsTpl =
        '<li class="tags__item"><a href="{{ path }}">{{ name }}</a></li>';
    ...
    render: function() {
      ...
      var result =
        this.config.language == "en"
          ? "Sorry,the content of your search does not exist!"
          : "抱歉，您要的内容似乎没有哦，不如换个关键字试试吧。";
      var self = this;
    ...
    filterSourceData: function() {
      ...
      this.result.forEach(function(row, idx) {
        if (
          self.config.minScore > row.score &&
          idx >= self.config.minScore.minNum
        ) {
          return;
        }
